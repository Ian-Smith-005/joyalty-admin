...
export async function onRequestPost(context) {
  ...
  if (!env.SUPABASE_URL) return j({ success: true, warning: "No DB configured" });

  let body;
  try { body = await request.json(); } catch (_) { return j({ error: "Invalid body" }, 400); }
  ...
  const db = sb(env);
  const { data, error } = await db.from("live_chat_messages")
    .insert({ session_id: sessionId, sender, name, text, timestamp: timestamp || new Date().toISOString() })
    .select("id").single();
  ...
  if (sender === "admin") {
    await db.from("live_chat_messages")
      .update({ read: true }).eq("session_id", sessionId).eq("sender", "user");
  }
