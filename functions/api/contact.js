...
export async function onRequestPost(context) {
  ...
  if (!env.RESEND_API_KEY) {
    return jsonRes(
      { error: "RESEND_API_KEY not configured in Cloudflare env vars" },
      500,
    );
  }
  ...
  var adminHtml =
    ...
    "<tr><td style='color:#6b7280;width:100px'>Name</td><td><strong>" +
    esc(name) +
    "</strong></td></tr>" +
    "<tr><td style='color:#6b7280'>Email</td><td><a href='mailto:" +
    esc(email) +
    "'>" +
    esc(email) +
    "</a></td></tr>" +
    "<tr><td style='color:#6b7280'>Phone</td><td>" +
    esc(phone || "Not provided") +
    "</td></tr>" +
    "<tr><td style='color:#6b7280'>Subject</td><td>" +
    esc(subject) +
    "</td></tr>" +
    "</table>" +
    "<hr style='margin:16px 0;border:none;border-top:1px solid #e5e7eb'>" +
    "<p style='color:#6b7280;font-size:.8rem;margin-bottom:6px'>MESSAGE</p>" +
    "<div style='background:#f9fafb;padding:14px;border-radius:6px;font-size:.9rem;line-height:1.7;white-space:pre-wrap'>" +
    esc(message) +
    "</div>" +
    ...
    "?subject=Re: " +
    esc(subject) +
    "' style='background:#1a1a2e;color:#fff;padding:10px 20px;border-radius:50px;text-decoration:none;font-size:.85rem;font-weight:700'>Reply to " +
    esc(name) +
    "</a>" +
    "</div>" +
    "</div></div>";
  ...
  var clientHtml =
    ...
    "<h2 style='color:#fff;margin:10px 0 4px'>Thank you, " +
    esc(name.split(" ")[0]) +
    "!</h2>" +
    "<p style='color:rgba(255,255,255,.6);margin:0;font-size:.88rem'>We received your message</p>" +
    "</div>" +
    "<div style='border:1px solid #e5e7eb;border-top:none;padding:24px 30px;border-radius:0 0 8px 8px'>" +
    "<p style='font-size:.92rem;line-height:1.75;color:#374151'>Hi " +
    esc(name.split(" ")[0]) +
    ", thank you for reaching out to <strong>Joyalty Photography</strong>. We have received your message and will get back to you within <strong>24 hours</strong> (Mon–Sat, 9AM–7PM).</p>" +
    "<div style='background:#f9fafb;padding:14px;border-radius:6px;margin:16px 0;border-left:3px solid #4f46e5;font-size:.88rem;white-space:pre-wrap'>" +
    esc(message) +
    "</div>" +
