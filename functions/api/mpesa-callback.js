...
function meta(items, name) {
  for (const i of items || []) { if (i.Name === name) return i.Value; }
  return null;
}
...
export async function onRequestPost(context) {
  ...
  let body;
  try { body = await request.json(); } catch (_) { return ok(); }
  ...
  if (!env.SUPABASE_URL) { console.error("[callback] SUPABASE_URL not set"); return ok(); }

  const db = sb(env);

  if (!confirmed) {
    await db.from("payments").update({ status: "failed" }).eq("mpesa_checkout_id", checkoutId);
    return ok();
  }

  const items    = cb.CallbackMetadata?.Item || [];
  const mpesaRef = meta(items, "MpesaReceiptNumber") || `SBX-${checkoutId.slice(-8).toUpperCase()}`;
  const paidAmt  = meta(items, "Amount");

  // 1. Mark payment completed
  await db.from("payments").update({
    status: "completed", mpesa_receipt: mpesaRef, completed_at: new Date().toISOString()
  ...
  // 4. Confirm booking
  await db.from("bookings").update({ status: "confirmed", updated_at: new Date().toISOString() }).eq("id", bid);
  ...
  if (env.RESEND_API_KEY) {
    const { data: receipt } = await db.from("receipts").select("*").eq("booking_id", bid).single();
    if (receipt) sendEmails(env, receipt).catch(e => console.error("[callback] email:", e.message));
  }
...
async function sendEmails(env, r) {
  ...
  const sends = [];
  if (r.client_email) {
    sends.push(resend(env.RESEND_API_KEY, { from: `Joyalty Photography <${from}>`, to: [r.client_email], subject: `Your booking is confirmed — ${r.receipt_ref}`, html: clientHtml }));
  }
  sends.push(resend(env.RESEND_API_KEY, { from, to: [admin], subject: `💰 New payment: ${r.receipt_ref} — ${r.client_name}`, html: adminHtml }));
