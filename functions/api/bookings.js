...
export async function onRequestPost(context) {
  ...
  if (!clientName || !clientEmail || !clientPhone || !serviceType)
    return j(
      {
        error:
          "Missing required fields: clientName, clientEmail, clientPhone, serviceType",
      },
      400,
    );
  ...
  const { data: svcs, error: sErr } = await db
    ...
    .limit(1);
  if (sErr || !svcs?.length)
    return j({ error: "Unknown service: " + serviceType }, 400);
  const service = svcs[0];

  const { data: pkgs } = await db
    .from("packages")
    .select("*")
    .eq("name", servicePackage)
    .limit(1);
  const pkg = pkgs?.[0];
  ...
  await db.from("receipts").insert({
    ...
    package_name: servicePackage,
    extra_name: extraServices,
    event_date: eventDate,
    event_time: eventTime,
